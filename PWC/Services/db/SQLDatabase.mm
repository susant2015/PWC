...
//================================================================================================================================================================================================
- (void)saveImage: (NSData*) imageData id: (int) nId
{
	sqlite3_stmt *statement;
    NSString *sql=@"insert into image_data('id', 'image_data') values(?, ?)";
    if( (statement = [self prepare:sql]) )
    {
        sqlite3_bind_int(statement, 1, nId);
        sqlite3_bind_blob(statement, 2, [imageData bytes], [imageData length], SQLITE_TRANSIENT);
        
        if(sqlite3_step(statement) != SQLITE_DONE ) {
            NSLog( @"Error: %s", sqlite3_errmsg([self dbh]));
        } else {
            NSLog( @"Insert into row id = %lld", (sqlite3_last_insert_rowid([self dbh])));
        }
...
- (NSData*) showImage: (int) nId
{
    ...
    NSData *imageData;
    NSString *insertSQL = [NSString stringWithFormat:@"Select image_data from image_data Where Id = %d", i];
    if(sqlite3_prepare_v2([self dbh],[insertSQL cStringUsingEncoding:NSUTF8StringEncoding], -1, &compiledStmt, NULL) == SQLITE_OK) {
