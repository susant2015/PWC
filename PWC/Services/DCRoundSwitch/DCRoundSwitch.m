...
- (void)sizeToFit
{
    ...
    CGSize sizeOnString = [onString sizeWithAttributes:
                   @{NSFontAttributeName:self.toggleLayer.labelFont}];
    
    // Values are fractional -- you should take the ceilf to get equivalent values
    CGSize adjustedOnStringSize = CGSizeMake(ceilf(sizeOnString.width), ceilf(sizeOnString.height));
    
    CGSize sizOffString = [offString sizeWithAttributes:
                           @{NSFontAttributeName:self.toggleLayer.labelFont}];
    CGSize adjustedOffStringSize = CGSizeMake(ceilf(sizOffString.width), ceilf(sizOffString.height));


	CGFloat width = adjustedOnStringSize.width;
	CGFloat offWidth = adjustedOffStringSize.width;
